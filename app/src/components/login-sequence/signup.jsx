...
export default function Signup() {
  const { setShowSignup, setShowForgetPass, setShowLogin, message, setShowVerify } =
    useShowPagesContext();
  const { buttonLoading, setButtonLoading } = useContext(activeTabContext);
  ...
  function handleTyping(e) {
    ...
    setFormData((prevData) => {
      ...
      if (name === "password" || name === "confirmPassword") {
        setPassMatch(newData.password === newData.confirmPassword);
      }
      return newData;
    });
  }
  ...
  async function handleSignup(e) {
    ...
    console.log('res:: ', res);
    setApiRes({status: res.status, message: res.message})
    if (res.status === true) {
      localStorage.setItem('needsVerify', JSON.stringify({email: formData.email, verified: false}));
      setShowVerify(true);
    }
    } catch(err) {
  ...
  return (
    <div className="h-[100vh]">
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 bg-[#121B32] rounded-3xl w-[50%] p-5">
        <h1 className="text-white flex flex-row justify-center text-[24px] font-bold py-7">
          {t("Create Account")}
        </h1>
        {apiRes && apiRes.status === false && apiRes.message.includes('The email has already been taken')  && (
              <p className="text-[#ED1C24] text-[14px] flex flex-row justify-center">
                {t(`${apiRes.message}`)}
              </p>
            )}
        <div className="flex flex-col items-center">
          ...
          >
            ...
            <div className="flex flex-col w-full">
              <label className="text-[#ACADB1]" htmlFor="name">
                {t("Full name")}
              </label>
              <input
            ...
            <div className="flex flex-col w-full">
              ...
              <input
                className={`text-[#ACADB1] bg-transparent rounded-lg border ${ apiRes.status === false ? 'border-[#ED1C24]' : 'border-[#48516B]' } px-2 py-3 w-full mt-1 stroke-none outline-none`}
                name="email"
                type="email"
            ...
            <div className="flex flex-col w-full mt-5">
              ...
              {/* Password Inner div */}
              <div className="flex flex-row items-center justify-between rounded-lg border border-[#48516B] px-2 py-3 mt-1">
                <input
            ...
            {/* Confirm password outer div */}
            <div className="flex flex-col w-full mt-5">
              <label className="text-[#ACADB1]" htmlFor="confirmpass">
                {t("Confirm Password")}
              </label>
              {/* Password Inner div */}
              <div
                className={`flex flex-row items-center justify-between rounded-lg border px-2 py-3 mt-1 ${
                  passMatch
                    ? "border-[#48516B]"
                    : "border-red-500 bg-[#f513133d]"
                }`}
              >
                <input
                  className="text-[#ACADB1] bg-transparent rounded-lg w-full stroke-none outline-none"
            ...
            <button
              className={`main-yellow w-3/5 mt-12 py-3 rounded-full text-[#0B101D] font-bold ${
                !passMatch ? "bg-gray-200 cursor-not-allowed" : ""
              }`}
              disabled={!passMatch}
              type="submit"
            >
        ...
        <p className="text-[#ACADB1] mt-18 flex flex-row justify-center">
